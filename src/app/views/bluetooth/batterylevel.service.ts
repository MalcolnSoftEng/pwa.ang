...
import { from, fromEvent, Observable, throwError, pipe } from 'rxjs';
  ...
  getBatteryLevel() {
    ...
    try {
      return this.ble
        ...
        .pipe(mergeMap((gatt: BluetoothRemoteGATTServer) => {
          return this.ble.getPrimaryService$(
            gatt,
            BatteryLevelService.GATT_PRIMARY_SERVICE
          );
        }))
        .pipe(mergeMap((primaryService: BluetoothRemoteGATTService) => {
          return this.ble.getCharacteristic$(
            primaryService,
            BatteryLevelService.GATT_CHARACTERISTIC_BATTERY_LEVEL
          );
        }))
        .pipe(mergeMap((characteristic: BluetoothRemoteGATTCharacteristic) => {
